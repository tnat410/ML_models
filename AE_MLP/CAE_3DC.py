...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
			nn.Linear(X*Y*Z,1000),nn.PReLU(),
			nn.Linear(1000,800),nn.PReLU(),
			nn.Linear(800,600),nn.PReLU(),
			nn.Linear(600,400),nn.PReLU(),
        ...
        self.decoder = nn.Sequential(
			...
			nn.Linear(400,600),nn.PReLU(),
			nn.Linear(600,800),nn.PReLU(),
			nn.Linear(800,1000),nn.PReLU(),
			nn.Linear(1000,X*Y*Z))	
...
def trainAE(data):		
	...
	for epoch in range(epoch_num):
		...
		w = autoencoder.state_dict()['encoder.12.weight']
...
def trainContAEList(state,data,file_part):		
	autoencoder = AutoEncoder()
